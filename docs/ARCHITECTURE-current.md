...
|---|---|
| `docs/` | 設計文件與測試資料參考表 |
| `docs/ARCHITECTURE.md` | 原始完整架構規劃（規則引擎、RAG、資料庫） |
| `docs/ARCHITECTURE-current.md` | 本文件，描述目前簡化架構 |
| `docs/test-data.md` | 主要測試資料整理表，供人員與 bot 查詢 |
| `data/rules/` | 未來放置結構化規則與領域知識，目前為預留目錄 |
| `data/samples/` | 未來放置結構化測試資料樣本，目前為預留目錄 |
| `data/sources/` | 原始參考文件（PDF、OneNote 匯出檔等輸入來源） |
...
1. **原始文件匯入**：從 OneNote、SharePoint、PDF 等處取得原始文件，放入 `data/sources/`。
2. **人工整理**：將原始文件中的測試資料、注意事項整理成 Markdown 表格，放入 `docs/test-data.md`。
3. **查詢使用**：開發者、測試人員或 AI bot 直接閱讀 `docs/test-data.md` 取得測試所需資訊。
